...
bool Graphe::chargerDepuisOSM(const std::string& fichierOSM)
{
    ...
    // 2️ Charger les <way> (routes) et créer les arêtes
    ...
    // 3️ Ajouter uniquement les noeuds connectés aux arêtes
    for (Noeud* n : noeudsUtiles)
        ajouterNoeud(n);

    // 4️ Définir des coordonnées X/Y cohérentes (en mètres)
    if (!noeudsUtiles.empty()) {
        double originLat = noeudsUtiles[0]->getLat();
        double originLon = noeudsUtiles[0]->getLon();

        for (Noeud* n : noeudsUtiles) {
            double mx, my;
            latLonToMeters(n->getLat(), n->getLon(), originLat, originLon, mx, my);
            n->setX(mx);
            n->setY(my);
        }
    }

    std::cout << "Chargement OSM terminé : " << noeudsUtiles.size()
              << " noeuds connectés, " << aretes.size() << " arêtes." << std::endl;
